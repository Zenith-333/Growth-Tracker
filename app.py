...
# Works whether the .pkl files sit in a "models/" subfolder or right next
# to app.py in the repo root.
...
def _resolve_path(filename):
    for d in MODEL_DIR_CANDIDATES:
        ...
        if os.path.exists(candidate):
            return candidate
    # Nothing found anywhere -- return the first candidate so the resulting
    # FileNotFoundError message still points somewhere sensible.
    return os.path.join(MODEL_DIR_CANDIDATES[0], filename)
...
def load_artifacts():
    filenames = {
        ...
        "hfa_model": "hfa_model.pkl",
        "hfa_scaler": "hfa_scaler.pkl",
        "hfa_label_encoder": "hfa_label_encoder.pkl",
        "bmi_model": "bmi_model.pkl",
        "bmi_scaler": "bmi_scaler.pkl",
        "bmi_label_encoder": "bmi_label_encoder.pkl",
    }
    return {key: joblib.load(_resolve_path(fname)) for key, fname in filenames.items()}
